...
import {FantasyLeagueStanding, LeagueAttributesFragment} from "../../../../graphql/generated/Client";
...
const getFilteredStanding = (allStandings: SearchableStanding[], searchValue: string): SearchableStanding[] => {
    if (!searchValue) {
        return [];
    }

    return allStandings
        .filter(({ teamName, playerName}) => teamName.toLowerCase().includes(searchValue) || playerName.toLowerCase().includes(searchValue))
        .map((standing) => ({
            ...standing,
        }));
}
...
const LeagueSearch: React.FC<LeagueSearchProps> = (props) => {
    ...
    const allStandings: SearchableStanding[] = useMemo(() => leagues.flatMap(
        (league) => league.standings.map((standing) => ({ ...standing, leagueId: league.id }))
        ), [props.leagues])

    // Save search value in state
    const [searchValue, setSearchValue] = useState("");
    const handleChange = useCallback(debounce((event: React.ChangeEvent<HTMLInputElement>) => setSearchValue(String(event.target.value).toLowerCase()), 50), []);
    ...
    return (
        <div>
            ...
            <div className={styles.searchSummary}>
                {searchValue !== "" && searchResultsSummary}
            </div>
            <div className={styles.searchResults}>
                {filteredStandings
                    .filter((_, idx) => idx < MAX_STANDINGS_TO_SHOW)
                    .map((searchResult) => (
                        <LeagueParticipant key={searchResult.teamId + searchResult.leagueId} standing={searchResult} />
                    ))
                }
            </div>
        </div>
