...
const getSummaryOfPerformance = (points: number): string => {
    if (points < -300) {
        return (
            '👑 Congratulations! You lost over 300 points due to your obsession with getting fancy. ' +
    ...
    if (points < -200) {
        return (
            '💎 You lost over 200 points due to your fancy ways. ' +
    ...
    if (points < -75) {
        return (
            '🍾 You lost over 75 points. ' +
            "Guessing you're not a Liverpool fan. Glass of champagne for the fancy lad?"
        );
    }

    if (points < -50) {
        return '🙅 You lost more than 50 points. ' + 'Couple of bad decisions? Palmer (c) in GW19?';
    }

    if (points < -25) {
        return (
            '🤦 You lost over 25 points. ' +
            'A couple of hits and misses, but unfortunately for you, they were mostly misses.'
        );
    }

    if (points === -14) {
        return '🤦 You lost 14 points. ' + "That's how many points Salah scored in his first game.";
    }

    if (points === -10) {
        return (
            '🤦 You lost 10 points. ' + "That's how many points Salah scored in his second game."
        );
    }

    if (points < -10) {
        return (
            '🍺 You lost over 10 points. ' +
    ...
    if (points < 0) {
        return (
            '☕ You lost under 10 points! ' + "You don't like to get fancy. We respect that here."
        );
    }
    ...
    if (points < 10) {
        return '🎉 You gained points! ' + 'Couple of lucky picks? Had Haaland at the start?';
    }

    if (points < 25) {
        return (
            '🎉 You gained points! ' + 'You made a couple of changes and it paid off. Good work!'
        );
    }
    ...
    if (points < 100) {
        return (
            '👑 You gained over 100 points! ' +
            "Somehow you're fancy and you're making it work. Keep it up!"
        );
    }

    if (points < 200) {
        return '👑 You gained over 200 points! ' + 'This guy, man. How do you do it?';
    }
};
