...
const fancyCalculator = async (managerId: FantasyManagerId): Promise<FancyResult> => {
    ...
    const fancyResults = await Promise.all(
        history.map(async (week) => {
            ...
            const pointDifference =
                (captainGameSummary?.points ?? 0) - (salahGameSummary?.points ?? 0);
    ...
    const totalPointDifference = fancyResults.reduce(
        (acc, result) => acc + result.pointDifference,
        0
    );
