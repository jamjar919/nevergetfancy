import { FancyComparison, FancyPickLine, FancyResultLine } from '../../../graphql/generated/Resolver';
...
const fancyGetPlayerComparisonFactory = (type: FancyComparisonTypeEnum): () => Promise<PremierLeaguePlayerId> => {
    switch (type) {
...
const fancyComparisonCalculator = async (
    ...
    const comparisonScores = await Promise.all(playerPicks.map(async (line: FancyPickLine): Promise<FancyResultLine> => {
        const comparisonPlayerId = await getPlayerToCompareTo();
        const comparisonGameSummary: PlayerPreviousGameDto | undefined = await getPlayerPreviousGame(
            comparisonPlayerId,
        ...
        return {
            ...
            points,
            pointDifference,
            comparisonGameSummary: comparisonGameSummary ? convertGameSummary(comparisonGameSummary) : undefined
        }
    }));
    ...
    const totalPointDifference = comparisonScores.reduce((acc: number, line: FancyResultLine) => acc + line.pointDifference, 0);
