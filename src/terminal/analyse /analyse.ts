...
const analyse = async (numPlayers: number, gameweek: EventId): Promise<void> => {
    ...
    await Promise.all(standings.map(async (standing, idx) => {
        try {
            if (idx % 250 === 0) {
            ...
            const { points: captainPoints } = (await getPlayerPreviousGame(captainedPlayer, gameweek))!;

            // record their best player
            const bestCaptainInTeam = await getBestCaptainPickInTeamForGameWeek(standing.teamId, gameweek);
            const { points: bestCaptainPoints } = (await getPlayerPreviousGame(bestCaptainInTeam, gameweek))!;
