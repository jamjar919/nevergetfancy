...
const exportTeamsDb = async () => {
    ...
    try {
        ...
        const minId = (
            sourceDatabase.prepare('SELECT MIN(id) as minId FROM fpl_teams') as any
        ).get().minId;
        const maxId = (
            sourceDatabase.prepare('SELECT MAX(id) as maxId FROM fpl_teams') as any
        ).get().maxId;

        for (let startId = minId; startId <= maxId; startId += batchSize) {
            const endId = Math.min(startId + batchSize - 1, maxId);
            const teams = sourceDatabase
                .prepare('SELECT * FROM fpl_teams WHERE id BETWEEN ? AND ?')
                .all(startId, endId);
            ...
            if (teams.length > 0) {
                ...
                const queryText =
                    'INSERT INTO fpl_teams (id, team, manager) VALUES ' +
